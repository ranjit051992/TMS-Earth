...
module.exports = {
    ...
    async convertToPO() {
        await I.waitForVisible("//a[contains(text(),'" + lmtVar.getLabel("CONVERT_TO_PO") + "')]");
        await I.waitForClickable("//a[contains(text(),'" + lmtVar.getLabel("CONVERT_TO_PO") + "')]");
        await I.click("//a[contains(text(),'" + lmtVar.getLabel("CONVERT_TO_PO") + "')]");
        await commonKeywordImpl.waitForLoadingSymbolNotDisplayed();
    ...
    async clickOnReturnButton() {
        await I.waitForVisible(I.getElement(iBuyersDeskObject.REQ_RETURN_BUTTON));
        await I.waitForClickable(I.getElement(iBuyersDeskObject.REQ_RETURN_BUTTON));
        await I.click(I.getElement(iBuyersDeskObject.REQ_RETURN_BUTTON));
    ...
    async clickOnConvertToPOButton() {
        await I.waitForVisible(I.getElement(iBuyersDeskObject.CONVERT_PO_BUTTON));
        await I.waitForClickable(I.getElement(iBuyersDeskObject.CONVERT_PO_BUTTON));
        await I.click(I.getElement(iBuyersDeskObject.CONVERT_PO_BUTTON));
    },

    async clickOnAddItemToExistingPOYesButton() {
        let noOfElements = await I.grabNumberOfVisibleElements("//span[contains(text(),'" + lmtVar.getLabel("YES") + "')]");
        if (noOfElements > 0) {
            //await I.waitForVisible("//span[contains(text(),'"+lmtVar.getLabel("Yes")+"')]");
            await I.waitForClickable("//span[contains(text(),'" + lmtVar.getLabel("YES") + "')]");
            await I.click("//span[contains(text(),'" + lmtVar.getLabel("YES") + "')]");
        }

    },

    async selectAllLineItems() {
        //await I.waitForVisible(I.getElement(iBuyersDeskObject.ITEM_HEADER_CHECKBOX));
        await I.waitForClickable(I.getElement(iBuyersDeskObject.ITEM_HEADER_CHECKBOX));
        await I.click(I.getElement(iBuyersDeskObject.ITEM_HEADER_CHECKBOX));
    },

    async selectResubmitReqOnReturnCheckbox() {
        // await I.waitForVisible(I.getElement(iBuyersDeskObject.RESUBMIT_CHECKBOX));
        await I.waitForClickable(I.getElement(iBuyersDeskObject.RESUBMIT_CHECKBOX));
        await I.click(I.getElement(iBuyersDeskObject.RESUBMIT_CHECKBOX));
    },

    async checkIfActionMenuPresent() {
        let isPresent = false;
        let noOfElements = await I.grabNumberOfVisibleElements(I.getElement(iBuyersDeskObject.ACTION_MENU_ICON));
        if (noOfElements > 0) {
            isPresent = true;
            logger.info("Actions menu is present.");
        }
        else {
            logger.info("Actions menu is not present.");
        }

        return isPresent;
    },

    async checkPrimaryActionPresent(action) {
        let isPresent = false;
        let noOfElements = await I.grabNumberOfVisibleElements("//a[contains(text(),'" + action + "')]");
        if (noOfElements > 0) {
            isPresent = true;
            logger.info(action + " Action is present.");
        }
        else {
            logger.info(action + " Actions menu is not present.");
        }
