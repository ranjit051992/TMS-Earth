...
export class EmailTest_NewImpl_TMS {
    ...
    static async readEmail(email: string, password: string, parameter: string[], value: string[], InboxValue: string, delay: number) {
        return new Promise(async (resolve, reject) => {
            ...
            imap.once('ready', async function () {
                await EmailTest_NewImpl_TMS.openInbox(InboxValue, imap, async function (err: any, box: any) {
                    if (err) {
                        ...
                        await EmailTest_NewImpl_TMS.delay(delay).then(async function () {
                            ...
                            imap.search(searchCondition, async function (err: any, results: any) {
                                if (!results || results.length == 0) {
                                    ...
                                    let f: any;
                                    f = imap.fetch(results[results.length - 1], { bodies: ['HEADER', 'TEXT'] });
                                    ...
                                    f.on('message', async function (msg: any, seqno: any) {
                                        ...
                                        parser.on('data', async (data: any) => {
                                            console.log(seqno);
                                            await EmailTest_NewImpl_TMS.fetchmailBody(data).then(async function (email) {
                                                // console.log("body    ", Startup.email_Test.get("emailBody"))
                                                await EmailTest_NewImpl_TMS.getLinksFromBody(Startup.email_Test.get("emailBody")).then(async function () {
                                                    await EmailTest_NewImpl_TMS.fetchListFromBody();
