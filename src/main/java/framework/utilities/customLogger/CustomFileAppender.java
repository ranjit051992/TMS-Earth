package framework.utilities.customLogger;
...
public class CustomFileAppender extends FileAppender
{
	...
	public CustomFileAppender(Layout layout, String fileName, boolean append, boolean bufferedIO,
	...
	public void append(LoggingEvent event)
	{
		try
		{
			...
			{
				setFile(appendLevelToFileName(MDC.get(ORIGINAL_FILE_NAME).toString(), "BrowserConsole"), fileAppend,
			...
			else
			{
				...
				if (event.getThrowableInformation() != null)
				{
					for (int i = 0; i < event.getThrowableInformation().getThrowableStrRep().length; i++)
					{
						...
						event.getThrowableInformation().getThrowableStrRep()[i].toLowerCase().contains("assertion"))
						{
							setFile(appendLevelToFileName((String) MDC.get(ORIGINAL_FILE_NAME), "Exception"), fileAppend,
									bufferedIO, bufferSize);
							super.append(event);
							break;
						}
